...
import {
  type Item, type Rollup, rollup, groupBy, isOutlier,
...
function worst(row: { value: number }[], w: number): number {
...
function layoutSquarified(nodes: TreeNode[], x: number, y: number, w: number, h: number, out: TreeNode[]) {
...
function treemapLayout(nodes: TreeNode[], x: number, y: number, w: number, h: number): TreeNode[] {
  const total = nodes.reduce((s, n) => s + Math.max(0, n.value), 0);
...
interface TreemapProps {
  items: Item[];
  level: DrillLevel;
  setLevel: (l: DrillLevel) => void;
  setFilters: React.Dispatch<React.SetStateAction<Filters>>;
  filters: Filters;
  pinnedSector: string | null;
}
...
export function Treemap({ items, level, setLevel, setFilters, filters, pinnedSector }: TreemapProps) {
  ...
  const groups = useMemo(() => {
    ...
    const nodes: TreeNode[] = [...g.entries()].map(([k, v]) => {
      const r = rollup(v);
  ...
  const tiles = useMemo(() => {
    ...
    return treemapLayout(groups, 0, 0, size.w, size.h);
  }, [groups, size]);

  const onClickTile = (node: TreeNode) => {
...
interface DonutProps {
  ...
  active?: string | null;
  onClick?: (s: DonutSlice) => void;
}

export function Donut({ slices, size = 180, inner = 64, onHover, active, onClick }: DonutProps) {
